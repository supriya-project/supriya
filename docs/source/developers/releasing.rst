...
The version information is stored in :github-tree:`supriya/_version.py
<supriya/_version.py>` as a tuple so that it's available by Supriya after
installation, and by :github-tree:`setup.py <setup.py>` during installation
...
This will bump the current version number in
:github-tree:`<supriya/_version.py>` and cut a draft of a new release in
GitHub.
