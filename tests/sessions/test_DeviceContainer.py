...
from supriya.ugens import In, ReplaceOut, SynthDefBuilder, system
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            +                1015 supriya:dc-tester:2 (session.mixers[0].devices[0]:synth-0)
            ...
            """,
            """
            - ['/d_recv', <SynthDef: supriya:dc-tester:2>]
            - ['/sync', 3]
            - [None, [['/g_new', 1014, 0, 1002], ['/s_new', 'supriya:dc-tester:2', 1015, 1, 1014, 'out', 16.0]]]
            """,
        ),
        (
            [
            ...
            +                        1015 supriya:dc-tester:2 (session.mixers[0].tracks[0].devices[0]:synth-0)
            ...
            - ['/d_recv', <SynthDef: supriya:dc-tester:2>]
            - ['/sync', 3]
            - [None, [['/g_new', 1014, 0, 1009], ['/s_new', 'supriya:dc-tester:2', 1015, 1, 1014, 'out', 18.0]]]
            """,
        ),
    ],
)
...
async def test_DeviceContainer_add_device(
    ...
    async with run_test(
        ...
        device = await target_.add_device(
            synth_configs=[SynthConfig(synthdef=system.build_dc_tester_synthdef)],
        )
...
@pytest.mark.asyncio
async def test_DeviceContainer_add_effect(
    commands: list[tuple[str | None, str, dict | None]],
    expected_components_diff: Callable[[Session], str] | str,
    expected_messages: str,
    expected_tree_diff: str,
    ...
    async with run_test(
        ...
        target_ = session[target]
        assert isinstance(target_, DeviceContainer)
        effect = await target_.add_device(
            name="Device",
            parameter_configs=parameter_configs,
            sidechain_configs=sidechain_configs,
            synth_configs=synth_configs,
        )
    assert isinstance(effect, Device)
    assert effect in target_.devices
    assert effect.parent is target_
    assert target_.devices[0] is effect
