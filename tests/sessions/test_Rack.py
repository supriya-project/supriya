...
@pytest.mark.parametrize(
    ...
    [
        SetChannelCountScenario(
            ...
            - [None,
               [['/s_new', 'supriya:patch-cable:2x4:replace', 1022, 0, 1011, 'in_', 16.0, 'out', 22.0],
...
def build_device_config(
    *, multiplier: float = 1.0, offset: float = 0.0
    ...
    return DeviceConfig(
        parameter_configs=[
            ParameterConfig(
                name="multiplier", field=FloatField(default=multiplier, has_bus=True)
            ),
            ParameterConfig(
                name="offset", field=FloatField(default=offset, has_bus=True)
            ),
        ],
        synth_configs=[
            SynthConfig(
...
@pytest.mark.parametrize(
    ...
    [
        SetReadModeScenario(
            ...
            - ['/sync', 3]
            - ['/s_new', 'supriya:zero:2', 1034, 0, 1014, 'out', 18.0]
            - ['/s_new', 'supriya:zero:2', 1035, 0, 1024, 'out', 18.0]
            - ['/n_set', 1016, 'done_action', 2.0, 'gate', 0.0]
            - ['/n_set', 1026, 'done_action', 2.0, 'gate', 0.0]
            """,
            expected_tree_diff="""
            --- initial
            +++ mutation
            @@ -12,8 +12,10 @@
                         1010 group (session.mixers[0].devices[1]:group)
                             1011 group (session.mixers[0].devices[1]:chains)
                                 1014 group (session.mixers[0].devices[1].chains[0]:group)
            +                        1034 supriya:zero:2 (session.mixers[0].devices[1].chains[0]:input)
            +                            out: 18.0
                                     1016 supriya:patch-cable:2x2:replace (session.mixers[0].devices[1].chains[0]:input)
            -                            active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 16.0, out: 18.0
            +                            active: 1.0, done_action: 2.0, gain: 0.0, gate: 0.0, in_: 16.0, out: 18.0
                                     1017 supriya:meters:2 (session.mixers[0].devices[1].chains[0]:input-levels)
                                         in_: 18.0, out: 14.0
                                     1015 group (session.mixers[0].devices[1].chains[0]:devices)
            @@ -29,8 +31,10 @@
                                     1019 supriya:meters:2 (session.mixers[0].devices[1].chains[0]:output-levels)
                                         in_: 18.0, out: 16.0
                                 1024 group (session.mixers[0].devices[1].chains[1]:group)
            +                        1035 supriya:zero:2 (session.mixers[0].devices[1].chains[1]:input)
            +                            out: 18.0
                                     1026 supriya:patch-cable:2x2:replace (session.mixers[0].devices[1].chains[1]:input)
            -                            active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 16.0, out: 18.0
            +                            active: 1.0, done_action: 2.0, gain: 0.0, gate: 0.0, in_: 16.0, out: 18.0
                                     1027 supriya:meters:2 (session.mixers[0].devices[1].chains[1]:input-levels)
                                         in_: 18.0, out: 24.0
                                     1025 group (session.mixers[0].devices[1].chains[1]:devices)
            """,
        ),
        SetReadModeScenario(
            ...
            - ['/d_recv', <SynthDef: supriya:patch-cable:2x2:replace>]
            - ['/sync', 3]
            - ['/s_new', 'supriya:patch-cable:2x2:replace', 1034, 0, 1014, 'in_', 16.0, 'out', 18.0]
            - ['/s_new', 'supriya:patch-cable:2x2:replace', 1035, 0, 1024, 'in_', 16.0, 'out', 18.0]
            - ['/n_free', 1016]
            - ['/n_free', 1026]
            """,
            expected_tree_diff="""
            ...
            @@ -12,8 +12,8 @@
                         1010 group (session.mixers[0].devices[1]:group)
                             1011 group (session.mixers[0].devices[1]:chains)
                                 1014 group (session.mixers[0].devices[1].chains[0]:group)
            -                        1016 supriya:zero:2 (session.mixers[0].devices[1].chains[0]:input)
            -                            out: 18.0
            +                        1034 supriya:patch-cable:2x2:replace (session.mixers[0].devices[1].chains[0]:input)
            +                            active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 16.0, out: 18.0
                                     1017 supriya:meters:2 (session.mixers[0].devices[1].chains[0]:input-levels)
                                         in_: 18.0, out: 14.0
                                     1015 group (session.mixers[0].devices[1].chains[0]:devices)
            @@ -29,8 +29,8 @@
                                     1019 supriya:meters:2 (session.mixers[0].devices[1].chains[0]:output-levels)
                                         in_: 18.0, out: 16.0
                                 1024 group (session.mixers[0].devices[1].chains[1]:group)
            -                        1026 supriya:zero:2 (session.mixers[0].devices[1].chains[1]:input)
            -                            out: 18.0
            +                        1035 supriya:patch-cable:2x2:replace (session.mixers[0].devices[1].chains[1]:input)
            +                            active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 16.0, out: 18.0
                                     1027 supriya:meters:2 (session.mixers[0].devices[1].chains[1]:input-levels)
                                         in_: 18.0, out: 24.0
                                     1025 group (session.mixers[0].devices[1].chains[1]:devices)
...
@pytest.mark.parametrize(
    ...
    [
        SetWriteModeScenario(
            ...
            - ['/sync', 3]
            - ['/s_new', 'supriya:patch-cable:2x2:mix', 1034, 3, 1011, 'active', 0.0, 'in_', 20.0, 'out', 16.0]
            ...
            @@ -45,8 +45,10 @@
                                         active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 18.0, out: 20.0
                                     1029 supriya:meters:2 (session.mixers[0].devices[1].chains[1]:output-levels)
                                         in_: 18.0, out: 26.0
            +                1034 supriya:patch-cable:2x2:mix (session.mixers[0].devices[1]:output)
            +                    active: 0.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 20.0, mix: 0.0, out: 16.0
                             1012 supriya:patch-cable:2x2 (session.mixers[0].devices[1]:output)
            -                    active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 20.0, out: 16.0
            +                    active: 1.0, done_action: 2.0, gain: 0.0, gate: 0.0, in_: 20.0, out: 16.0
                             1013 supriya:meters:2 (session.mixers[0].devices[1]:output-levels)
                                 in_: 16.0, out: 10.0
                     1003 supriya:channel-strip:2 (session.mixers[0]:channel-strip)
            """,
        ),
        SetWriteModeScenario(
            ...
            - ['/d_recv', <SynthDef: supriya:patch-cable:2x2:mix>]
            - ['/sync', 3]
            - ['/s_new', 'supriya:patch-cable:2x2:mix', 1034, 3, 1011, 'in_', 20.0, 'mix', 'c9', 'out', 16.0]
            - ['/n_set', 1012, 'done_action', 2.0, 'gate', 0.0]
            """,
            expected_tree_diff="""
            --- initial
        ...
        SetWriteModeScenario(
            ...
            expected_messages="""
            - ['/s_new', 'supriya:patch-cable:2x2:replace', 1034, 3, 1011, 'in_', 20.0, 'out', 16.0]
            - ['/n_set', 1012, 'done_action', 2.0, 'gate', 0.0]
            """,
            expected_tree_diff="""
            --- initial
            ...
            @@ -45,8 +45,10 @@
                                     ...
                                     1029 supriya:meters:2 (session.mixers[0].devices[1].chains[1]:output-levels)
                                         in_: 18.0, out: 26.0
            +                1034 supriya:patch-cable:2x2:replace (session.mixers[0].devices[1]:output)
            +                    active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 20.0, out: 16.0
                             1012 supriya:patch-cable:2x2 (session.mixers[0].devices[1]:output)
            -                    active: 1.0, done_action: 2.0, gain: 0.0, gate: 1.0, in_: 20.0, out: 16.0
            +                    active: 1.0, done_action: 2.0, gain: 0.0, gate: 0.0, in_: 20.0, out: 16.0
