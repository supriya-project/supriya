...
def test_nonrepeatable(paths):
    ...
    with session.at(0):
        session.add_synth(
            duration=1,
            synthdef=nonrepeatable_noise_synthdef,
            )
    exit_code, output_file_path = session.render()
    pytest.helpers.assert_soundfile_ok_2(output_file_path, exit_code, 1, 44100, 1)
...
def test_repeatable(paths):
    ...
    with session.at(0):
        session.add_synth(
            duration=1,
            synthdef=repeatable_noise_synthdef,
            )
    exit_code, output_file_path = session.render()
    pytest.helpers.assert_soundfile_ok_2(output_file_path, exit_code, 1, 44100, 1)
    sampled_session = pytest.helpers.sample_soundfile(output_file_path)
...
def test_maybe_repeatable_but_wasnt(paths):
    ...
    with session.at(0):
        session.add_synth(
            duration=1,
            synthdef=maybe_repeatable_noise_synthdef,
            )
    exit_code, output_file_path = session.render()
    pytest.helpers.assert_soundfile_ok_2(output_file_path, exit_code, 1, 44100, 1)
    sampled_session = pytest.helpers.sample_soundfile(output_file_path)
...
def test_repeatable_via_session_method(paths):
    ...
    d_recv_commands = pytest.helpers.build_d_recv_commands([
        ...
        maybe_repeatable_noise_synthdef,
        ])
    assert session.to_lists() == [
        [0.0, [
            *d_recv_commands,
            ['/s_new', maybe_repeatable_noise_synthdef.anonymous_name,
                1000, 0, 0,
                'rand_seed', 0],
