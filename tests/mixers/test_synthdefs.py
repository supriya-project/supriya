...
from supriya.mixers.synthdefs import (
    build_channel_strip,
    build_patch_cable,
    get_lag_time,
)
...
async def test_free_channel_strip(
    ...
    """
    with server.at():
        with server.add_synthdefs(build_channel_strip(2)):
            group = server.add_group()
            synth = group.add_synth(synthdef=build_channel_strip(2))
    await server.sync()
...
async def test_free_patch_cable(
    ...
    """
    with server.at():
        with server.add_synthdefs(build_patch_cable(2, 2)):
            group = server.add_group()
            synth = group.add_synth(synthdef=build_patch_cable(2, 2))
    await server.sync()
    initial_tree = str(await server.query_tree())
    with server.at():
        synth.set(done_action=done_action)
