...
from supriya.commands import NodeInfo
...
async def test_move_node(context):
    ...
    synth = context.add_synth(default)
    with context.osc_protocol.capture() as transcript:
        group.move(target=synth, add_action="ADD_AFTER")
        synth.move(target=group, add_action="ADD_BEFORE")
        synth.move(target=group, add_action="ADD_TO_TAIL")
        synth.move(target=group, add_action="ADD_TO_HEAD")
    assert transcript.filtered(received=False, status=False) == [
...
async def test_query_node(context):
    ...
    assert await get(group.query()) == NodeInfo(
        action=NodeAction.NODE_QUERIED, is_group=True, node_id=1000, parent_id=1
    )
