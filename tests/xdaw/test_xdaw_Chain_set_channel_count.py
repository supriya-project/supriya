...
def test_channel_count(channel_count_application):
    ...
    channel_count_application["Chain"].set_channel_count(4)
    assert {
        x.name: (x.channel_count, x.effective_channel_count) for x in items[1:]
    } == {
        "Chain": (4, 4),
                        ...
                        1012 group (Rack)
                            ...
                            1015 group (ChainContainer)
                                1016 group (Chain)
                                    ...
                                    1023 group (Receives)
                                    1087 mixer/track-input/4 (Input)
                                        in_: 22.0, out: 44.0, active: 1.0, gain: 0.0, gate: 1.0, lag: 0.01
                                    1088 mixer/levels/4 (InputLevel)
                                        out: 44.0, gate: 1.0, lag: 0.01
                                    1024 group (Devices)
                                        1025 group (Device)
                                            1093 mix/patch[replace]/4x4 (DeviceIn)
                                                active: 1.0, gate: 1.0, in_: 44.0, lag: 0.01, out: 48.0
                                            1026 group (Parameters)
                                            1027 group (Body)
                                                1092 15a77f27cd96f4488c4a9b094478d045
                                                    out: 48.0, index: 0.0
                                            1094 mix/patch[hard,mix]/4x4 (DeviceOut)
                                    ...
                                    1090 mixer/track-output/4 (Output)
                                        out: 44.0, active: 1.0, gain: 0.0, gate: 1.0, hard_gate: 1.0, lag: 0.01
                                    1032 group (PostFaderSends)
                                        1095 mix/patch[gain]/4x2 (Send)
