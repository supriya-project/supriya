...
class TestCase(TestCase):
    ...
    def test_attributes(self):
        ...
        with session.at(0):
            buffer_one = session.add_buffer(
                frame_count=512,
                )
        with session.at(5):
            buffer_two = session.add_buffer(
                channel_count=2,
                duration=10,
                )
        # buffer one
    ...
    def test_copy_from(self):
        session = nonrealtimetools.Session()
        with session.at(0):
            buffer_group = session.add_buffer_group(
                buffer_count=2,
                frame_count=512,
                )
        with session.at(1):
    ...
    def test_fill_via_chebyshev(self):
        session = nonrealtimetools.Session()
        with session.at(0):
