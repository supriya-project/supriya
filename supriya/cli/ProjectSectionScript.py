...
class ProjectSectionScript(ProjectPackageScript):
    ...
    def _handle_copy(self, source_name, target_name, force=False):
        self._copy_package(source_name, target_name, self._section_plural, force=force)
    ...
    def _handle_new(self, name, force=False):
        self._create_package_from_template(
            package_name=name, section=self._section_plural, force=force
        )

    def _handle_rename(self, source_name, target_name, force=False):
        self._rename_package(
            source_name, target_name, self._section_plural, force=force
        )
    ...
    def _import_objects(self, section_plural, verbose=True):
        ...
        for path in self._list_subpackages(section_plural):
            name = path.name
            object_ = self._import_object(path, self._section_singular, verbose=verbose)
            objects[name] = object_
        return objects

    def _process_args_inner(self, args):
        if args.new is not None:
            self._handle_new(force=args.force, name=args.new)
        if args.edit is not None:
            self._handle_edit(names=args.edit)
        if args.list_:
            self._handle_list()
        if args.copy is not None:
            self._handle_copy(
                source_name=args.copy[0], target_name=args.copy[1], force=args.force
            )
        if args.rename is not None:
            self._handle_rename(
                source_name=args.rename[0], target_name=args.rename[1], force=args.force
            )
        if args.delete is not None:
            self._handle_delete(name=args.delete)
        if args.render is not None:
            self._handle_render(force=args.force, names=args.render)
