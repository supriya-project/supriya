...
class Track(DeviceContainer[TrackContainer], TrackContainer):
    ...
    def _delete(self) -> None:
        if self.parent:
            self.parent._unsolo_tracks(tracks=self._soloed_tracks)
        super()._delete()
    ...
    def _move(self, *, parent: TrackContainer, index: int) -> None:
        ...
        if self.mixer is not parent.mixer:
            ...
            raise RuntimeError
        elif (old_parent := self.parent) is None:
            raise RuntimeError
    ...
    def _set_soloed(self, *, soloed: bool, exclusive: bool = True) -> "Session":
