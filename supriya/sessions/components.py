...
class Component(Generic[C]):
    ...
    async def _reconcile(
        ...
        *,
        ...
        for root in reconciling_components:
            ...
            for component in root._walk(Component):
    ...
    def _walk(self, component_class: Type[T]) -> Generator[T, None, None]:
        if isinstance(self, component_class):
            yield self
        for child in self.children:
    ...
    async def dump_tree(
        ...
        """
        if self._ensure_session().status != BootStatus.ONLINE:
            raise RuntimeError
        tree = await cast(
        ...
        if annotation:
            ...
            for component in self._walk(Component):
